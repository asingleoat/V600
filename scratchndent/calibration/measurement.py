...
def normalize_transmittance(
    ...
    """
    img = raw_rgb.astype(np.float32)

    if dark_rgb is None:
        dark = np.zeros(3, dtype=np.float32)
    else:
        dark = np.asarray(dark_rgb, dtype=np.float32)

    if light_rgb is None:
        if raw_rgb.dtype == np.uint16:
            light = np.full(3, 65535.0, dtype=np.float32)
        else:
            light = np.full(3, 1.0, dtype=np.float32)
    else:
        light = np.asarray(light_rgb, dtype=np.float32)
